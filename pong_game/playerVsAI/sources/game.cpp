...
namespace AA
{
	...
	Game::Game()  //constructor for Game (mostly used to initiate the vars in the class)
	{
		...
		computerSpeed = 5;  //sets the speed for the computer platform movements
	...
	void Game::buildGame()  //update the screen and pieces values
	{
		if (ballLaunched==false)
		{
			...
			platform.updatePlatformsPosition(renderer, &plat1, &plat2, &ball, ballLaunched, &computerSpeed);  //change the values of the platforms
			reset = b.updateBallPosition(renderer, &ball, ballLaunched, &plat1, &plat2, &p1Score, &p2Score, &computerSpeed);  //change the values of the ball and checks if no one scored by the reset var		
